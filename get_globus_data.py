...
def get_globus_data(auth_token,transfer_token, gconfig):
    ...
    for ep in tc.endpoint_search(filter_scope="administered-by-me"):
        if ep["id"] in gconfig["interested_dtn_id_list"]:
        ...
        #    print("IF condition triggered: " + ep["id"])
        #    continue
...
def read_data_and_create_maps(endpoint_map_list_of_tasks, gconfig):
    ...
    #pp.pprint(endpoint_map_list_of_tasks)
    ...
    for endpoint in endpoint_map_list_of_tasks.keys():
        print("This should print x times: x = " + str(len(endpoint_map_list_of_tasks.keys())))
...
def main():
    ...
    # print raw data to blank file, for reuse later
    with open("raw_endpoint_data", "w") as fp:
        fp.write(str(endpoint_map_list_of_tasks))
